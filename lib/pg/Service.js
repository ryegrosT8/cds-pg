...
module.exports = class PostgresDatabase extends cds.DatabaseService {
  ...
  async acquire(arg) {
    ...
    // Set session variable to provide multi-tenant RLS strategy by session variable policy.
    // https://aws.amazon.com/pt/blogs/database/multi-tenant-data-isolation-with-postgresql-row-level-security/ (alternative approach)
    if (tenant) {
      dbc.query(`SET app.current_tenant = '${arg.user.tenant}';`)
    }
  ...
  getDbUrl() {
    ...
    const dbUrl = `postgres://${credentials.host}//${credentials.database}`
